...
for (const [dir, file] of QUIZZES) {
  ...
  for (const g of (all.questionnaireConfig?.questionGroups || [])) {
    if (!g.label && g.key) allConfigStrings.add(g.key.charAt(0).toUpperCase() + g.key.slice(1).replace(/_/g, ' '));
  }
}

const orphans = Object.keys(dict).filter((k) => !allConfigStrings.has(k));
if (orphans.length) {
