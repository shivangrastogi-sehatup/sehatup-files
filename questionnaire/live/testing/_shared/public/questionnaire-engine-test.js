...
class QuestionnaireEngine {
    constructor(config) {
        this.config = config;
        this.version = "3.6.4"; // Deep Sync & Cache Busting
        this.db = window.db;
        console.log("ENGINE VERSION 3.6.4 LOADED");
    ...
    async init() {
        ...
        const hideWithDelay = () => {
            ...
            // Only use delay if we didn't already hide it or if we are not on Step 1
    ...
    loadYoutubeVideo(target) {
        ...
        // controls=1 (native), iv_load_policy=3 (no annotations), cc_load_policy=0 (captions off)
        ...
        // 4. Advanced State Management (Hides thumbnail and badge ONLY when playing)
        const checkPlaying = (e) => {
            // Ensure the message is from our current iframe
            ...
            try {
                ...
                // Track both raw infoDelivery and official onStateChange events
                let state = -1;
                if (data.event === 'onStateChange') {
                    state = data.info;
                } else if (data.event === 'infoDelivery' && data.info) {
                    state = data.info.playerState !== undefined ? data.info.playerState : data.info;
                }
        ...
        // Immediate cleanup if the iframe is removed
