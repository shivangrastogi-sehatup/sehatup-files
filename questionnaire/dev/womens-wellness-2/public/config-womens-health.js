...
const questionnaireConfig = {
    ...
    productRules: (score, allAnswers, productDatabase, userInfo, config) => {
        ...
        if (score < 25) {
            baseProductNames = ['Ovinorm', 'Estrogen Balance', 'Vitamin D3 600UI'];
        } else if (score >= 25 && score <= 60) {
            baseProductNames = ['Ovinorm', 'Estrogen Balance', 'Vitamin D3 600UI'];
        } else if (score >= 61 && score <= 80) {
            baseProductNames = ['Ovinorm', 'Estrogen Balance', 'Vitamin D3 600UI'];
        } else {
            baseProductNames = ['Vitamin D3 600UI'];
        }

        let extraProductNames = [];
        const hasPMS =
        ...
        if (hasPMS) extraProductNames.push('HormoniHerb Tea');
        if (hasBadOdor) extraProductNames.push('Intimate Wash');
        if (hasComorbidities) extraProductNames.push('Thyroidinum 3x');

        const allNames = [...new Set([...baseProductNames, ...extraProductNames])];
        return allNames.map((name) => {
            const product = productDatabase[name];
            if (product && product.active === true) {
                let correctName = name;
                if (name === 'Ovinorm') correctName = 'Her Menses';
                if (name === 'Estrogen Balance') correctName = 'EstroEssentia';
                if (name === 'Vitamin D3 600UI') correctName = 'Zencal D3K2';
                if (name === 'Thyroidinum 3x') correctName = 'Thyrostatin 3X';
                const finalName = product.name || correctName;
                return { ...product, name: finalName, active: true };
            }
            return null;
        }).filter(Boolean); 
    },
    ...
    saveSubmission: async (state, db, config) => {
        ...
        const causeMapping = config.causeMapping || {};
        for (const groupKey in allAnswers) {
            allAnswers[groupKey].forEach((answer) => {
                const qCauses = causeMapping[answer.question];
                if (qCauses) {
                    const texts = Array.isArray(answer.text)
                        ? answer.text
                        : [answer.text];
                    texts.forEach((text) => {
                        const cause = qCauses[text];
        ...
        const lifestyleAnswers = allAnswers.lifestyle || [];
        const hormonalAnswers = allAnswers.hormonal || [];
        const hygieneAnswers = allAnswers.hygiene || [];

        // const productWhyPoints = config.productWhyPoints || {};
        ...
        const data = {
            ...
            possibleCauses: possibleCauses.map((cause) => ({
                text: cause,
            })),

            lifestyleChanges: lifestyleTipsArray,
            timeline: combinedTimeline,
            concern: 'Womens Personal Wellness',
            answers: answers,
            reportCategory: "Womens Personal Wellness",
            lifestyleAnswers: lifestyleAnswers,
            hormonalAnswers: hormonalAnswers,
            hygieneAnswers: hygieneAnswers,
