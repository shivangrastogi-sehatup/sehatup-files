...
class QuestionnaireEngine {
    ...
    async init() {
        ...
        // Check for previously completed report
    ...
    saveLocalState() {
        localStorage.setItem(`active_session_state_${this.config.id}`, JSON.stringify(this.state));
    }
            ...
            try {
                ...
                if (this.state.partialDocId) {
                    try {
                        ...
                        localStorage.removeItem(`active_session_state_${this.config.id}`); // Clear active session on finish
    ...
    renderResults() {
        ...
        if (userNameEl) userNameEl.innerText = this.state.userInfo.name;
        ...
        if (this.state.userInfo.dob) {
            ...
            if (userAgeEl) userAgeEl.innerText = age;
        }
        if (userConcernEl) userConcernEl.innerText = this.state.results.issueTitle;
        if (issueHeaderEl) issueHeaderEl.innerText = this.state.results.issueTitle;
        if (conditionTextEl) conditionTextEl.innerHTML = this.state.results.conditionTextHTML;
        if (conditionDetailsEl) conditionDetailsEl.innerHTML = this.state.results.conditionTextHTML;
        ...
        if (riskLevelEl && typeof this.config.getRiskType === 'function') {
            const riskType = this.config.getRiskType(this.state.healthScore);
            riskLevelEl.innerText = riskType;
    ...
    animateScore(finalScore) {
        ...
        const interval = setInterval(() => {
            ...
            // Modern score structure: <span class="score-num">0</span><span class="percent-sign">%</span>
            progressValue.innerHTML = `<span class="score-num">${currentScore}</span><span class="percent-sign">%</span>`;
            let color = this.getScoreColor(currentScore);
            progressBar.style.setProperty('--progress', `${currentScore}%`);
            progressBar.style.background = `conic-gradient(${color} var(--progress), #f3f4f6 0%)`;
        }, 20);
    }
