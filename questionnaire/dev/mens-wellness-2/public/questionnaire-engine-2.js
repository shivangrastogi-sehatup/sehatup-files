...
class QuestionnaireEngine {
    ...
    async init() {
        window.scrollTo(0, 0); // Force scroll to top on load/reload
        ...
        // Only show default step if we didn't recover a final report
        if (!this.state.finalDocId) {
            this.showStep(this.state.currentStep);
            
            // If we're at Step 1 and not recovering, we should hide the immediate loader
            // that was shown by default in Liquid/CSS to prevent refresh flickers.
    ...
    showStep(step) {
        ...
        if (step === resultsStep) {
            if (this.isSaving || this.state.finalDocId) {
                console.log("Submission already in progress or finished. Skipping redundant trigger.");
                this.renderResults();
                return;
            }
            ...
            if (contentContainer) {
                contentContainer.innerHTML = `
                    <div class="loading-results-card" style="text-align: center; padding: 60px 20px;">
                        <div class="loader-spinner" style="margin: 0 auto 20px;"></div>
                        <h2 style="margin-bottom: 10px;">Generating Your Report</h2>
                        <p style="color: #666;">Please wait while we calculate your health score...</p>
                    </div>
                `;
            }
            
            this.showLoader(); // Show the full screen animated loader too
            ...
            }, 3000); // Give loader room to breathe
            return;
        }
    ...
    scrollToTopIfMobile() {
        if (this.isInitializing) return; // Don't scroll during initial load
    ...
    finishQuestionnaire() {
        if (this.isSaving || this.state.finalDocId) return;
        this.isSaving = true;
        ...
        const saveToFirebase = async () => {
            try {
                ...
                // SAVE COMPLETED ID TO LOCAL STORAGE FOR RECOVERY
                localStorage.setItem(`completedDocId_${this.config.id}`, finalDocId);
                this.state.isVerified = false; // Initial state
    ...
    renderResults() {
        const questionnaireEl = document.getElementById('questionnaire');
        if (questionnaireEl) {
            questionnaireEl.classList.add('results-mode');
            questionnaireEl.classList.add('full-screen-results');
        }
        
        document.getElementById('user-name').innerText = this.state.userInfo.name || "Guest";
            ...
            timelineHTML += `
                <div class="month-box">
                    <div class="month-icon">
                        <img src="https://cdn.shopify.com/s/files/1/0924/5687/8383/files/image_277.png?v=1741590675" alt="${monthKey}" />
                    </div>
                    <h4>${monthKey}</h4>
        ...
        this.updateResultCTA();
    }

    updateResultCTA() {
        const ctaContainer = document.querySelector('.whatsapp-button-container');
        if (!ctaContainer) return;

        if (this.state.isVerified) {
            ctaContainer.innerHTML = `
                <button class="doctor-consult-btn" data-action="open-consultation">
                    <img src="https://cdn-icons-png.flaticon.com/512/3304/3304567.png" alt="Doctor" style="width: 20px; filter: invert(1);">
                    Talk to a Doctor Now
                </button>
            `;
            // Add handler for consultation if needed, or simple redirect
            const btn = ctaContainer.querySelector('button');
            btn.onclick = () => {
                window.open(`https://wa.me/919355539355?text=Hi, I have completed my ${this.state.results.issueTitle} report and I want a doctor consultation.`, '_blank');
            };
        } else {
            ctaContainer.innerHTML = `
                <button class="whatsapp-report-btn" data-action="open-otp">
    ...
    async loadState() {
        ...
        const completedDocId = localStorage.getItem(`completedDocId_${this.config.id}`);

        // 1. PRIORITY: If there's a completed report, show it!
        if (completedDocId) {
            try {
                this.showLoader();
                const docSnap = await this.db.collection('questionnaire_submissions').doc(completedDocId).get();
                if (docSnap.exists) {
                    const data = docSnap.data();
                    // Recover state safely - merge rawState into default structure
                    if (data.rawState) {
                        this.state = { ...this.state, ...data.rawState };
                    } 
                    
                    // Fallback/Legacy recovery or ensure userInfo is populated from top-level fields
                    this.state.userInfo = this.state.userInfo || {};
                    if (!this.state.userInfo.name) {
                        this.state.userInfo.name = data.userName || data.name || "";
                        this.state.userInfo.phone = data.phone || "";
                        this.state.userInfo.dob = data.dob || "";
                    }
                    
                    if (!this.state.allAnswers || Object.keys(this.state.allAnswers).length === 0) {
                        this.state.allAnswers = data.allAnswers || { concern: [] };
                    }
                    this.state.finalDocId = completedDocId;
                    this.state.isVerified = data.isVerified || false;

                    this.state.healthScore = this.config.calculateScore(this.state.allAnswers, this.state.userInfo, this.config);
                    this.state.recommendedProducts = this.config.productRules(this.state.healthScore, this.state.allAnswers, this.config.productDatabase, this.state.userInfo, this.config);
                    this.state.results = this.config.resultRules(this.state.healthScore, this.state.allAnswers, this.config, this.state.userInfo);

                    // Smooth reveal: wait a bit so user sees the GIF loader
                    await new Promise(resolve => setTimeout(resolve, 2500));
                    
                    this.hideLoader();
                    this.renderResults();
                    
                    // Force UI to results page
                    document.getElementById('questionnaire').classList.add('full-screen-results');
                    document.querySelectorAll('.step').forEach(s => s.classList.remove('active'));
                    document.getElementById('result-page').classList.add('active');
                    return;
                } else {
                    localStorage.removeItem(`completedDocId_${this.config.id}`);
                    this.hideLoader();
                }
            } catch (e) {
                console.error("Error loading completed report:", e);
                this.hideLoader();
            }
        }

        // 2. SECONDARY: Resume partial journey
        if (!partialDocId) return;

        this.showResumeModal("Welcome back!", "Do you want to resume your previous session?", "Resume", "Restart", async (shouldResume) => {
            if (shouldResume) {
                try {
                    const docSnap = await this.db.collection('partial_submissions').doc(partialDocId).get();
                    if (docSnap.exists) {
                ...
                try { await this.db.collection('partial_submissions').doc(partialDocId).delete(); } catch (e) { }
                localStorage.removeItem(`partialDocId_${this.config.id}`);
                this.showStep(1);
            }
        });
    ...
    restartJourney() {
        if (confirm("Are you sure you want to restart? Your current report will be cleared.")) {
            localStorage.removeItem(`partialDocId_${this.config.id}`);
            localStorage.removeItem(`completedDocId_${this.config.id}`);
    ...
    showLoader() { if (typeof window.showLoader === 'function') { window.showLoader(); } else { document.getElementById('loader').style.display = 'flex'; } }
    hideLoader() { if (typeof window.hideLoader === 'function') { window.hideLoader(); } else { document.getElementById('loader').style.display = 'none'; } }
    closeModal(target) {
    ...
    renderUGCContent() {
        ...
        ugcContainer.innerHTML = `
            ...
            <div class="ugc-slider" id="ugc-slider">
                ${['68SbZuINym0', 'EyvZLDLxFYU', 'i_lfAg9o4HA'].map(id => `
                    <div class="ugc-slide ugc-slide-lite" data-action="load-youtube" data-src="https://www.youtube.com/embed/${id}">
                        <div class="verified-badge">
                            <svg class="verify-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                                <path d="M20 6L9 17L4 12" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                            Verified Reviewer
                        </div>
                        <img class="ugc-thumbnail" src="https://i.ytimg.com/vi/${id}/sddefault.jpg" alt="Review">
        ...
        // Add scroll listener for mobile zoom effect
        const slider = document.getElementById('ugc-slider');
        if (slider) {
            slider.addEventListener('scroll', () => this.handleUGCScroll(slider));
            // Trigger once to set initial state
            this.handleUGCScroll(slider);
        }
    }

    handleUGCScroll(slider) {
        ...
        slides.forEach(slide => {
            ...
            const distanceFromCenter = Math.abs(sliderCenter - slideCenter);
            
            // Only apply on mobile/small screens where horizontal scroll is active
            if (window.innerWidth <= 768) {
                const maxDistance = slider.offsetWidth / 1.5;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (this.state.finalDocId) {
                    ...
                    console.log("✅ Final submission updated with verified phone and status.");
                }

                this.renderResults(); // Re-render to show Consultation CTA
