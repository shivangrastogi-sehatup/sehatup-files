...
export default function TeleSalesView({ onLogout, roles = [] }) {
    ...
    const getSavedFilters = () => {
        try {
            const saved = localStorage.getItem('telesales_dashboard_filters');
    ...
    const [rowsPerPage] = useState(saved.rowsPerPage || 12);
    ...
    useEffect(() => {
        ...
        localStorage.setItem('telesales_dashboard_filters', JSON.stringify(stateToSave));
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="container">
                <header className="header">
                    <div onClick={() => setCurrentView("dashboard")} style={{ cursor: 'pointer' }}>
                        <div className="h-title">
                            Tele-Sales <span style={{ fontWeight: 300, opacity: 1 }}>Portal</span>
                        </div>
                        <div style={{ color: "var(--muted)", marginTop: 6 }}>
                            Customer Information Tracking v1.0
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                        {(auth.currentUser?.email || '').includes('admin') || (auth.currentUser?.email || '').includes('shivang') ? (
                            <div className="btn-group" style={{ height: 40, padding: 3, background: 'rgba(255,255,255,0.03)', borderRadius: 12, display: 'flex' }}>
                                <button 
                                    ...
                                    style={{ padding: '0 16px', height: '100%', display: 'flex', alignItems: 'center', gap: 6, color: currentBackend === 'live' ? '#fff' : 'rgba(255,255,255,0.4)', fontWeight: currentBackend === 'live' ? 800 : 500, textShadow: currentBackend === 'live' ? '0 0 10px rgba(74, 222, 128, 0.5)' : 'none', background: currentBackend === 'live' ? 'rgba(74, 222, 128, 0.1)' : 'transparent', borderRadius: 10, border: 'none', cursor: 'pointer' }}
                                >
                                ...
                                <button 
                                    ...
                                    style={{ padding: '0 16px', height: '100%', display: 'flex', alignItems: 'center', gap: 6, color: currentBackend === 'dev' ? '#fff' : 'rgba(255,255,255,0.4)', fontWeight: currentBackend === 'dev' ? 800 : 500, textShadow: currentBackend === 'dev' ? '0 0 10px rgba(251, 191, 36, 0.5)' : 'none', background: currentBackend === 'dev' ? 'rgba(251, 191, 36, 0.1)' : 'transparent', borderRadius: 10, border: 'none', cursor: 'pointer' }}
                                >
                        ...
                        <div className="profile-dropdown-container">
                            ...
                            <div className="profile-dropdown-menu">
                                <div className="profile-dropdown-item" onClick={() => { setProfileTab("profile"); setCurrentView("profile"); }}>
                                    <User size={16} /> Profile Settings
                                </div>
                                <div className="profile-dropdown-item" onClick={() => { setProfileTab("history"); setCurrentView("profile"); }}>
                        ...
                        <button 
                            className="btn ghost" 
                            onClick={onLogout} 
                            style={{ background: 'rgba(239, 68, 68, 0.08)', color: '#fca5a5', border: '1px solid rgba(239, 68, 68, 0.2)', transition: 'all 0.3s ease', fontWeight: 600, fontSize: 13, padding: '8px 20px', borderRadius: 100 }}
                        >
                            Logout
                        </button>
                    </div>
                        ...
                        <div className="panel glass-panel" style={{ display: 'flex', flexDirection: 'column', minHeight: '600px' }}>
                            <div className="crm-toolbar crm-toolbar-primary">
                                <div className="crm-search">
                                    ...
                                    <input type="text" placeholder="Search by name, phone, or symptom..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} />
                                    {searchQuery && <button type="button" className="crm-search-clear" onClick={() => { setSearchQuery(''); setCurrentPage(1); }}><X size={14} /></button>}
                                </div>
                                <div className="crm-daterange">
                                    <input type="date" className="crm-daterange-input" value={toIsoDate(fromDate)} max={toIsoDate(toDate) || TODAY_ISO} onChange={(e) => { const v = e.target.value; if (!v) { setFromDate(null); setCurrentPage(1); return; } setFromDate(new Date(v)); setCurrentPage(1); }} />
                                    <span className="crm-daterange-arrow">→</span>
                                    <input type="date" className="crm-daterange-input" value={toIsoDate(toDate)} min={toIsoDate(fromDate)} max={TODAY_ISO} onChange={(e) => { const v = e.target.value; if (!v) { setToDate(null); setCurrentPage(1); return; } setToDate(new Date(v)); setCurrentPage(1); }} />
                                </div>
                            </div>

                            <div className="crm-toolbar crm-toolbar-secondary">
                                <div className="crm-chip-group">
                                    <button className={`crm-chip chip-full ${activeCollection === 'questionnaire_submissions' ? 'active' : ''}`} onClick={() => setActiveCollection(prev => prev === 'questionnaire_submissions' ? 'all' : 'questionnaire_submissions')}>Full</button>
                                    <button className={`crm-chip chip-partial ${activeCollection === 'partial_submissions' ? 'active' : ''}`} onClick={() => setActiveCollection(prev => prev === 'partial_submissions' ? 'all' : 'partial_submissions')}>Partial</button>
                                    <button className={`crm-chip chip-manual ${activeCollection === 'manual_submissions' ? 'active' : ''}`} onClick={() => setActiveCollection(prev => prev === 'manual_submissions' ? 'all' : 'manual_submissions')}>Manual</button>
                                    <span className="crm-chip-sep" />
                                    <button className={`crm-chip chip-consulted ${consultedOnly ? 'active' : ''}`} onClick={() => { setConsultedOnly(!consultedOnly); setCurrentPage(1); }}>Consulted</button>
                                    <button className={`crm-chip chip-purchased ${purchasedOnly ? 'active' : ''}`} onClick={() => { setPurchasedOnly(!purchasedOnly); setCurrentPage(1); }}>Purchased</button>
                                    <button className={`crm-chip chip-whatsapp ${whatsappOnly ? 'active' : ''}`} onClick={() => { setWhatsappOnly(!whatsappOnly); setCurrentPage(1); }}>WhatsApp</button>
                                </div>
                                <div className="crm-controls">
                                    <div className="crm-control-group">
                                        ...
                                        <button className={`crm-order-toggle ${sortOrder === 'asc' ? 'asc' : 'desc'}`} onClick={() => setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc')}>{sortOrder === 'asc' ? '↑' : '↓'}</button>
                                    </div>
                                    <div className="crm-records-count"><strong>{processedSubmissions.length.toLocaleString()}</strong> records</div>
                                    <ExportControls filtered={{ completed: processedSubmissions, partial: [] }} showStatus={showStatus} />
                                </div>
                            </div>
                                ...
                                {loading ? (
                                    ...
                                    <div className="patient-grid">
                                        {paginatedSubmissions.map(p => (
                                            <div key={p.id} className={`patient-item ${p._collection}`} onClick={() => setSelectedUser(p)}>
                                                <div className={`p-avatar ${p.isPurchased ? 'purchased' : ''}`}><User size={14} /></div>
                                                <div className="p-info">
                                                    <div className="p-name-row">
                                                        <div className="p-name">{p.userName || p.name || 'Anonymous Patient'}</div>
                                                        {p.isConsulted && <div className="consulted-badge">Consulted</div>}
                                                    </div>
                                                    <div className="p-meta">
                                                        <span><Phone size={14} /> {p.phone || 'No Phone'}</span>
                                                        <span><CalIcon size={14} /> {formatTableTimestamp(p.timestamp)}</span>
                                                    </div>
                                                </div>
                            ...
                            {totalPages > 1 && (
                                <div className="pagination-wrapper">
                                    <div className="pagination-info">Showing {((currentPage - 1) * rowsPerPage) + 1} to {Math.min(currentPage * rowsPerPage, processedSubmissions.length)} of {processedSubmissions.length}</div>
                                    <div className="pagination-btns">
                                        <button className="btn ghost mini-btn" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}><ChevronLeft size={18} /></button>
                                        ...
                                        <button className="btn ghost mini-btn" disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)}><ChevronRight size={18} /></button>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                {currentView === "profile" && <DoctorProfile defaultTab={profileTab} onTabChange={setProfileTab} />}

                <svg style={{ position: 'absolute', width: 0, height: 0 }}>
                    <defs>
                        <linearGradient id="scoreGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                            <stop offset="0%" stopColor="var(--accent1)" /><stop offset="100%" stopColor="var(--accent2)" />
                        </linearGradient>
                    </defs>
                </svg>
            </div>
            ...
            {selectedUser && currentView === "dashboard" && (
                <PatientDetailModal
                    ...
                    collectionName={selectedUser._collection === 'full' ? 'questionnaire_submissions' : (selectedUser._collection === 'manual' ? 'manual_submissions' : 'partial_submissions')}
