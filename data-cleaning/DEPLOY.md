...
- `build-data.mjs` — regenerates `public/training-data/` from `../training-data/`
...
Add it to Vercel (run from `data-cleaning/`, PowerShell):
```
vercel login
vercel link                         # create/link the project, root dir = data-cleaning
...
2. **Set Root Directory = `data-cleaning`** (Settings → General).
